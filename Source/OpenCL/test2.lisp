...
#|
;; #TODO: Da invalid program executable -11 al hacer clBuildProgram
(setf *program-source*
...
(setf *program-source*
"__kernel void Fitness(__global const float* a, __global const float* b, __global float* c) { int iGID = get_global_id(0); float x = a[iGID]; float y = sin(x); c[0] = 2332; }")
...
(fli:with-dynamic-foreign-objects
  ...
  (setf (fli:dereference local-work-size) #(1))
  (check-cl
...
(let ((x-values (convert-float-foreign-array *samples-x*)))
  ...
  (setf (aref *lisp-result-buffer* 0) 0.0)
  (check-cl 
...
;OCL 101 : 1 > (create-write-buffer *context* (* *float-size* *result-buffer-size*) *lisp-buffer*)
;return-code #<Pointer: BUFFER = #x00000000>
;error-code -37
;#<Pointer: BUFFER = #x00000000>
