...
(defmethod enqueue-kernel (kernel queue global-work-size local-work-size)
  (fli:with-dynamic-foreign-objects
    ...
    (setf (fli:dereference global-work-size) global-work-size
          (fli:dereference local-work-size) local-work-size)
    (check-cl (|clEnqueueNDRangeKernel| queue kernel 1 0 global-work-size local-work-size 0 nil nil))))
...
(defvar *lisp-result-buffer* (make-array *buffer-size* :element-type 'single-float :allocation :static))
...
(setf *result-buffer-size* 8)
(setf *global-work-size* 8)
(setf *local-work-size* 1)


(setf *program-source* (string-from-file #P"d:\\temp\\source.cl"))
;(setf *program-source* (list "__kernel void Fitness(__global const float* a, __global const float* b, __global float* c) { c[0] = 2332; " "}"))
...
(setf *result-buffer* (create-write-buffer *context* (* *float-size* *result-buffer-size*) *lisp-buffer*))
(fli:with-dynamic-foreign-objects
    ((error ocl:buffer))
...
(let ((x-size (* *float-size* *source-buffer-size*)))
  ...
  (setf *source-buffer-x* (create-write-buffer *context* x-size))
  (dotimes (i *source-buffer-size*) (setf (aref *lisp-buffer* i) 0.0))
  ;; Copy values to lisp-array
  (check-cl 
   (|clEnqueueWriteBuffer| *queue* *source-buffer-x* 1 1 (1- x-size) *lisp-buffer* 0 nil nil))
  ;; Create array for y values and clear
  (setf *source-buffer-y* (create-write-buffer *context* x-size))
  (dotimes (i *source-buffer-size*) (setf (aref *lisp-buffer* i) 0.0))
  ;; Copy values to lisp-array
  (check-cl 
   (|clEnqueueWriteBuffer| *queue* *source-buffer-y* 1 0 (1- x-size) *lisp-buffer* 0 nil nil)))

;; Kernel
(fli:with-dynamic-foreign-objects
    ((global-work-size ocl::size-t)
     (local-work-size ocl::size-t)) 
  (setf (fli:dereference global-work-size) #(1)
        (fli:dereference local-work-size) #(1))
  (dotimes (i 10)
    (check-cl
     (|clEnqueueNDRangeKernel| *queue* *kernel* 1 0 global-work-size local-work-size 0 nil nil))))

;;; Read result
(let ((x-values (convert-float-foreign-array *samples-x*)))
  ;; Create array for result values, clear and queue read action
  (setf (aref *lisp-buffer* 0) 0.0)
  (check-cl 
   (|clEnqueueReadBuffer| *queue* *result-buffer* 1 0 (* 4 *result-buffer-size*) *lisp-buffer* 0 nil nil)))
